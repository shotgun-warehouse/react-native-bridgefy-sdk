...
RCT_REMAP_METHOD(init, startWithApiKey:(NSString *)apiKey successCallback:(RCTResponseSenderBlock)successCallback errorCallBack:(RCTResponseSenderBlock)errorCallBack) {
  if (self.transmitter != nil) {
    NSDictionary * dictionary = [self createClientDictionary];
    successCallback(@[[NSNull null],dictionary]);
  }
...
RCT_EXPORT_METHOD(stop) {
  ...
  [self sendEventWithName:kStopped body:nil];
}
